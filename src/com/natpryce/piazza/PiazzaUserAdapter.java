...
public class PiazzaUserAdapter implements UserModelListener {
    ...
    final void reloadUsers() {
        this.userGroup = new UserGroup();
        Set<SUser> users = userModel.getAllUsers().getUsers();
        for (SUser user : users) {
            addUserToPiazzaMonitor(user, PiazzaNotificator.getPortraitUrl(user));
        }
    ...
    private void addUserToPiazzaMonitor(SUser user, String portraitUrl) {
        ...
        PiazzaUser piazzaUser = new PiazzaUser(user.getDescriptiveName(), vcsNicknames, user.getUsername(), portraitUrl);
